...
<ul>
	...
	<li><code>maxBoxes</code> and <code>maxWeight</code> are the respetive box and weight limit of the ship.</li>
</ul>

<p>The boxes need to be delivered <strong>in the they are given</strong>. The ship will follow these steps:</p>

<ul>
	<li>The ship will take some number of boxes from the <code>boxes </code>  queue, not  violating the <code>maxBoxes</code> and <code>maxWeight</code> constraints.</l
	<li>For each loaded box <strong>in order</strong>, theship wll make a <strong>trip</strong> to the port the box needs to be delivered to and deliver it. If the ship is already at the correct port, no <strong>tri p< /stong> is needed, and  the box can immediately be delivered.</li>
	<li>The ship then makes a return <strong>trip</strong> to storage  to task e more box e from the queue.</li>
</ul>
